...
{		
	...
	inline bool operator!=(const NumberStyles& x, int y) { return static_cast<int>(x) != y; }	
	**/
	AddEnumFlagSupport(NumberStyles,int);
	...
	template<class T> inline bool Core_Float_TryParse(const char *pszString, NumberStyles Style, T& Value, int& ParsedCount)
	{
		...
		// Look for special values, as generated by StringConversion.h's float/double routines.
		if (IsEqualNoCase(pp, "NaN")) { Value = std::numeric_limits<T>::quiet_NaN(); return true; }
		if (IsEqualNoCase(pp, "+inf") || IsEqualNoCase(pp, "inf")) { Value = std::numeric_limits<T>::infinity(); return true; }
		if (IsEqualNoCase(pp, "-inf")) { Value = ((T)-1.0) * std::numeric_limits<T>::infinity(); return true; }
		...
		if ((*pp >= '0' && *pp <= '9') || *pp == '.')
		{
			...
			if (bExponent)
			{
				...
				Value = (T)pow((double)Value, dExpValue);
			}
	...
	inline bool Float_TryParse(const string& string, NumberStyles Style, float& Value)
	{
		int ParsedCount;
		return Core_Float_TryParse<float>(string.c_str(), Style, Value, ParsedCount);
	}

	inline bool Double_TryParse(const string& string, NumberStyles Style, double& Value)
	{
		int ParsedCount;
		return Core_Float_TryParse<double>(string.c_str(), Style, Value, ParsedCount);
	}

	inline float Float_Parse(const string& string, NumberStyles Style)
	{
		float Value;
		if (!Float_TryParse(string, Style, Value)) throw FormatException();		
		return Value;
	}

	inline double Double_Parse(const string& string, NumberStyles Style)
	{
		double Value;
		if (!Double_TryParse(string, Style, Value)) throw FormatException();		
		return Value;
	}
	...
	{
		if (IsEqualNoCase(psz, S("true")) ||
			IsEqualNoCase(psz, S("yes")) || 
			IsEqualNoCase(psz, S("on"))) { Value = true; return true; }
		if (IsEqualNoCase(psz, S("false")) ||
			IsEqualNoCase(psz, S("no")) || 
			IsEqualNoCase(psz, S("off"))) { Value = false; return true; }
	...
	inline bool Number_TryParse(const string& string, NumberStyles Style, double& Value) { return Double_TryParse(string, Style, Value); }
	inline bool Number_TryParse(const string& string, NumberStyles Style, float& Value) { return Float_TryParse(string, Style, Value); }
	inline bool Number_TryParse(const string& string, NumberStyles Style, Int64& Value) { return Int64_TryParse(string, Style, Value); }
	inline bool Number_TryParse(const string& string, NumberStyles Style, UInt64& Value) { return UInt64_TryParse(string, Style, Value); }
	inline bool Number_TryParse(const string& string, NumberStyles Style, Int32& Value) { return Int32_TryParse(string, Style, Value); }
	inline bool Number_TryParse(const string& string, NumberStyles Style, UInt32& Value) { return UInt32_TryParse(string, Style, Value); }
	inline bool Number_TryParse(const string& string, NumberStyles Style, Int16& Value) { return Int16_TryParse(string, Style, Value); }
	inline bool Number_TryParse(const string& string, NumberStyles Style, UInt16& Value) { return UInt16_TryParse(string, Style, Value); }
	inline bool Number_TryParse(const string& string, NumberStyles Style, Int8& Value) { return Int8_TryParse(string, Style, Value); }
	inline bool Number_TryParse(const string& string, NumberStyles Style, UInt8& Value) { return UInt8_TryParse(string, Style, Value); }

	template<typename T> inline T Number_Parse(const string& string, NumberStyles Style) { 
		T Value;
		if (!Number_TryParse(string, Style, Value)) throw FormatException();
		return Value;
	}
}
